...
def get_cash_flow(
    ...
    """
    Returns closing balances from all bank statements over time.
    Used by the Cash Flow chart on the dashboard.
    """
    ...
    ).order_by(BankStatement.period.asc()).all()

    return [
        {
            "period": s.period,
            "closing_balance": s.closing_balance or 0,
        }
        for s in statements
    ]
